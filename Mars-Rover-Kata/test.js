...
// *the obstacles are generated randomly, and the test cases will vary in fuction of this.
    ...
    roverInit(3,5,myRover); //true: This Rover is ready!
    roverInit(6,8,roverClone); //true: This Rover is ready! The coordenates are: [1,2]
...
// Testing grid
    //--Individual Options
    gridInit(5,5); //true: the grid is created
...
// Testing rover
    //--Creating myRover or cloneRover if it is into the grid coordenates
    gridInit(5,5); //true: Grid Created!
    ...
    //--Creating myRover or cloneRover without a grid created
    roverInit(3,3,myRover); //true: Sorry! You need to create a grid before

    //--Creating myRover or cloneRover with negative coordenates
    gridInit(5,5); //true: Grid Created!
    roverInit(-3,3,myRover); //true: Sorry! You can't use negative numbers!

    //--Creating myRover or cloneRover if it isn't into the grid coordenates
    gridInit(5,5); //true: Grid Created!
    ...
    //--Creating myRover or cloneRover in the same place where other rover is already exist
    gridInit(5,5); //true: Grid Created!
    roverInit(2,2,myRover); //true: This Rover is ready!
    roverInit(2,2,roverClone); //true: Ooops! there is other rover here

    //--Creating myRover or cloneRover and this is already created
    gridInit(5,5); //true: Grid Created!
    roverInit(2,2,myRover); //true: This Rover is ready!
    roverInit(1,3,myRover); //true: Ooops! This rover is already created!

    //--Creating myRover or cloneRover in the same position of an obstacle created before
    gridInit(5,5); //true: Grid Created!
